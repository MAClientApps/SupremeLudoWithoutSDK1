...
Game.prototype = {
	create: function() {
		...
		btn_pause = new Button(this.game, this.game.world.centerX + gradle.btn_pause.x + 50,  30+gradle.btn_pause.y, "but_go_menu", main.storage.data.bg, this.pauseBtnClicked, !0);
		
		btn_pause.width = gradle.btn_pause.width;
		btn_pause.height = gradle.btn_pause.height;
		btn_pause.properties.gameManager = this;
		btn_pause.scale.setTo(0.4, 0.4);
		btn_pause.anchor.setTo(0.5, 0.5);
		
		//button two players :
		this.soundButton = this.add.button(this.world.centerX - 50, 30+gradle.btn_pause.y, 'but_music_yes', this.set_music_state, this, 1, 0, 2);
        this.soundButton.scale.setTo(0.4, 0.4);
	...
	set_music_state: function(){
		//main.managers.storage.save();

		console.log('sound change state ...');

		main.managers.audio.toggleMusic();	
		main.managers.audio.toggleSounds();

		gradle.enable_music = !gradle.enable_music;
		this.soundButton = this.add.button(this.world.centerX - 50, 30+gradle.btn_pause.y, 'but_music_'+(gradle.enable_music?'yes':'no'), this.set_music_state, this, 1, 0, 2);
		this.soundButton.scale.setTo(0.4, 0.4);
		this.soundButton.anchor.setTo(0.5, 0.5);
	...
	pauseBtnClicked: function() {
		...
		main.managers.audio.playSound("menu_click");
		main.dialog.show(main, main.dialog.type.yesNo, main.managers.language.get("quit_text"), null,
			null,
			function() {
	...
	exitToMenu: function() {
		//main.game.continueGame=true;
		/*main.game.self.activePlayer.state == main.player.state["throw"] || main.game.self.activePlayer.type == main.player.type.human ? (main.game.self.saveGame(), main.game.self.savePending = false, main.game.self.game.state.start("Intro")) : (main.game.self.savePending = false, main.game.self.game.state.start("Intro"));*/
		main.game.self.savePending = false;
		main.game.self.game.state.start("Intro");
		this.onGameOver(GAME_OVER_BY_USER);
		
		gradle.event('exit_to_menu');
	},
	...
	new_game(){
		...
		this.scoreTable = [];
		b = -1;
		for (d = 0; d < main.game.maxPlayers; d++) {
			a = new Player(this.game, this, this.board, c.playerTypesMenu[d], c.playerNamesMenu[d], d);
			-1 == b && (a.type == main.player.type.human || 0 == main.storage.data.playerTypesCount[main.player.type.human] && a.type == main.player.type.ai) && (this.activePlayerID = b = d, this.board.setup(b)), main.game.players.push(a);
		}
	...
	playerWin: function(a) {
		...
		gradle.score += 50;
		localStorage.setItem('ludo_score', gradle.score);
		gradle.save_score(gradle.score, 0);
	...
	turnDone: function(a) {
		...
		this.savePending ? (this.saveGame(), this.game.state.start("Intro")) : (this.activePlayer.state =
...
Intro.prototype = {
	...
	create: function() {
		...
		main.dialog.maxTextWidth = this.world.width - 200;
		
		this.bg_menu = this.game.add.image(0, 0, "bg_1");
		this.bg_menu.width = this.world.width;
		this.bg_menu.height = this.world.height;
		this.bg_menu.x = 0;
		this.bg_menu.y = 0;
		...
		//button one player :
		this.oneplayButton = this.add.button(this.world.centerX, 300, 'but_play', this.start_one_player, this, 1, 0, 2);
        this.oneplayButton.scale.setTo(0.7, 0.7);
        this.oneplayButton.anchor.setTo(0.5, 0.5);
		
		this.textoneplayer = this.game.add.text(this.world.centerX, 300, main.managers.language.get("one_player"), {
			font: '24px Arial',
			fill: main.game.colors_strings[0],
			align: "center"
		});
		this.textoneplayer.anchor.setTo(0.5, 0.5);
		
		//button two players :
		this.oneplayButton = this.add.button(this.world.centerX, 450, 'but_play', this.start_two_players, this, 1, 0, 2);
        this.oneplayButton.scale.setTo(0.7, 0.7);
        this.oneplayButton.anchor.setTo(0.5, 0.5);
		
		this.textoneplayer = this.game.add.text(this.world.centerX, 450, main.managers.language.get("two_players"), {
			font: '24px Arial',
			fill: main.game.colors_strings[0],
			align: "center"
		});
	...
	start_one_player: function(){
		var c = main.storage.data;
		gradle.shuffle(gradle.players_names);
		var text_player = main.managers.language.get("player");
		
		//console.log('config one player ...');
		c.playerTypesGame[0] = c.playerTypesMenu[0] = 0;
		c.playerTypesGame[1] = c.playerTypesMenu[1] =1;
		c.playerTypesGame[2] = c.playerTypesMenu[2] =1;
		c.playerTypesGame[3] = c.playerTypesMenu[3] =1;
		
		c.playerNamesGame[0] = c.playerNamesMenu[0] = text_player;
		c.playerNamesGame[1] = c.playerNamesMenu[1] = gradle.players_names[0];
		c.playerNamesGame[2] = c.playerNamesMenu[2] = gradle.players_names[1];
		c.playerNamesGame[3] = c.playerNamesMenu[3] = gradle.players_names[2];
		main.managers.storage.save();
		//console.log('start on player ...');
		this.state.start("Game");
	},
	
	start_two_players: function(){
		var c = main.storage.data;
		gradle.shuffle(gradle.players_names);
		var text_robot1 = main.managers.language.get("robot");
		var text_robot2 = main.managers.language.get("robot");
		var text_player = main.managers.language.get("player");
		
		c.playerTypesGame[0] = c.playerTypesMenu[0] =0;
		c.playerTypesGame[1] = c.playerTypesMenu[1] =1;
		c.playerTypesGame[2] = c.playerTypesMenu[2] =0;
		c.playerTypesGame[3] = c.playerTypesMenu[3] =1;
		
		c.playerNamesGame[0] = c.playerNamesMenu[0] = text_player +' 1';
		c.playerNamesGame[1] = c.playerNamesMenu[1] = gradle.players_names[0];
		c.playerNamesGame[2] = c.playerNamesMenu[2] = text_player +' 2';
		c.playerNamesGame[3] = c.playerNamesMenu[3] = gradle.players_names[1];
		main.managers.storage.save();
		this.state.start("Game");
	},
...
Menu.prototype = {
	create: function() {
		...
		console.log(main.dialog.maxTextWidth);
...
Preloader.prototype = {
	preload: function() {
		...
		this.game.load.image("bg_1", "images/bg_trans.png");
		this.game.load.image("logo", "images/logo.png");
		this.game.load.image("board", "images/plocha.png");
		this.game.load.atlasJSONHash("atlas", "images/atlas.png", "images/atlas.json");

		this.game.load.image('but_go_menu', 'images/but_go_menu.png');
        this.game.load.image('but_play', 'images/but_play.png');
        this.game.load.image('but_music_yes', 'images/but_music_yes.png');
        this.game.load.image('but_music_no', 'images/but_music_no.png');
		
		var a, b;
		for (b in main.sprites) main.sprites.hasOwnProperty(b) && (a = main.sprites[b], this.game.load.spritesheet(b, "images/" + a.path, a.w, a.h));
...
Player.prototype = {
	...
	enterGame: function(a) {
		...
		this.state = main.player.state.wait;
		this.place = -1;
		for (var b = this.tokensInFinish = 0; b < main.token.count; b++) 
			a = new Token(this.game, this.gameManager, this, this.board), a.skipTo(main.board.HOME_SQUARES[this.position] + b, !1), this.tokens.push(a);
